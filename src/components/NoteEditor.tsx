...
export default function NoteEditor({ note, onClose }: { note: Note; onClose: () => void }) {
  ...
  // ⚠️ VÉRIFICATIONS ULTRA STRICTES
  if (!menuItems) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          <div className="space-y-4">
            ...
            {categories.map(({ value, label }) => {
              const categoryItems = menuItems.filter(m => m.category === value);
                ...
                <div key={value} className="bg-white shadow-md rounded-lg p-3 sm:p-4">
                  ...
                  <div className="space-y-2">
                    {categoryItems.map(menuItem => {
                      ...
                      return (
                        <div key={menuItem.id} className="flex items-center justify-between bg-gray-50 rounded p-2 sm:p-3 border border-accent">
                          <div className="flex-1 min-w-0 mr-2">
                            <p className="text-gray-900 font-medium text-sm sm:text-base truncate">{menuItem.name}</p>
                            <p className="text-accent text-xs sm:text-sm">{menuItem.price.toFixed(2)}€</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <button onClick={() => removeItem(menuItem.id)} disabled={note.status === 'cloture'} className="bg-red-500 hover:opacity-90 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed text-white w-8 h-8 rounded font-bold text-lg">−</button>
                            <span className="text-accent font-bold text-base sm:text-lg w-6 text-center">{quantity}</span>
                            <button onClick={() => addItem(menuItem.id)} disabled={note.status === 'cloture'} className="bg-accent-light hover:opacity-90 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed text-white w-8 h-8 rounded font-bold text-lg">+</button>
                          </div>
                        </div>
                      );
                    })}
          ...
          <div className="space-y-4">
                  ...
                  {items.map(item => {
                    ...
                    return (
                      <div key={item.menuItemId} className="flex justify-between items-center text-gray-900 text-sm sm:text-base">
                        ...
                        {note.status === 'ouvert' && (
                          <button onClick={() => removeItem(item.menuItemId)} className="ml-3 bg-red-500 hover:opacity-90 text-white w-7 h-7 rounded font-bold text-sm flex items-center justify-center">−</button>
                        )}
                      </div>
                    );
                  })}
            ...
            {tempRemovals.length > 0 && (
              <div className="bg-orange-50 border-2 border-orange-400 rounded-lg p-3 sm:p-4">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <button onClick={clearTempRemovals} className="text-red-500 hover:text-red-700 text-xl font-bold">✕</button>
                </div>
                <div className="space-y-2">
            ...
            <div className="bg-white shadow-md rounded-lg p-3 sm:p-4">
              <div className="flex justify-between items-center cursor-pointer" onClick={() => setShowAdjustments(!showAdjustments)}>
                <h4 className="text-accent font-semibold text-sm sm:text-base">AJUSTEMENTS</h4>
                <button className="text-accent text-xl font-bold">{showAdjustments ? '−' : '+'}</button>
              </div>
              {showAdjustments && (
                <div className="mt-3">
                  {note.status === 'ouvert' && (
                    <div className="mb-4 space-y-2">
                      <input type="number" step="0.01" placeholder="Montant (positif pour ajouter, négatif pour retirer)" value={adjustmentAmount} onChange={(e) => setAdjustmentAmount(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && addAdjustment()} className="w-full bg-white text-gray-900 border-2 border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-accent text-sm" />
                      <button onClick={addAdjustment} className="w-full bg-accent hover:opacity-90 text-white py-2 rounded font-semibold transition text-sm">AJOUTER</button>
                    </div>
                  )}
                    ...
                    <div className="space-y-2">
                      {adjustments.map(adj => (
                        <div key={adj.id} className="flex justify-between items-center bg-gray-50 rounded p-2 text-sm">
                          <p className="text-gray-600 text-xs">{new Date(adj.timestamp).toLocaleString('fr-FR')}</p>
                          <div className="flex items-center gap-2">
                            <span className={`font-semibold ${adj.amount >= 0 ? 'text-accent-light' : 'text-red-500'}`}>{adj.amount >= 0 ? '+' : ''}{adj.amount.toFixed(2)}€</span>
                            {note.status === 'ouvert' && <button onClick={() => removeAdjustment(adj.id)} className="text-red-500 hover:opacity-80">✕</button>}
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="bg-white shadow-md rounded-lg p-3 sm:p-4">
              <div className="flex justify-between items-center cursor-pointer" onClick={() => setShowDivision(!showDivision)}>
                <h4 className="text-accent font-semibold text-sm sm:text-base">DIVISER LA NOTE</h4>
                <button className="text-accent text-xl font-bold">{showDivision ? '−' : '+'}</button>
              </div>
              {showDivision && (
                <div className="mt-3 flex flex-col sm:flex-row items-start sm:items-center gap-3">
                  <label className="text-gray-900 text-sm font-medium whitespace-nowrap">Nombre de personnes :</label>
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    <input type="number" min="1" value={numberOfPeople} onChange={(e) => setNumberOfPeople(e.target.value)} placeholder="1" className="w-20 bg-gray-50 text-gray-900 border-2 border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-accent text-center font-bold" />
                    <span className="text-gray-900 text-sm">→</span>
                    <div className="flex-1 bg-accent-light bg-opacity-10 rounded px-4 py-2 border-2 border-accent-light">
                      <span className="text-accent-light font-bold text-base sm:text-lg">{numberOfPeople && parseInt(numberOfPeople) > 0 ? (total / parseInt(numberOfPeople)).toFixed(2) : total.toFixed(2)}€ / pers.</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="space-y-2">
              {note.status === 'ouvert' ? (
                <button onClick={handleCloseNote} className="w-full bg-red-500 hover:opacity-90 text-white py-3 rounded font-semibold transition text-sm sm:text-base">CLÔTURER LA NOTE</button>
              ) : (
                <button onClick={() => reopenNote(note.id)} className="w-full bg-accent-light hover:opacity-90 text-white py-3 rounded font-semibold transition text-sm sm:text-base">ROUVRIR LA NOTE</button>
              )}
            </div>
          </div>
        </div>
