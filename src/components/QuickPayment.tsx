                      ...
                      return (
                        <div key={menuItem.id} className="flex items-center justify-between bg-gray-50 rounded p-2 sm:p-3 border border-accent">
                          ...
                          <div className="flex items-center gap-2">
                            <button onClick={() => removeItem(menuItem.id)} className="bg-red-500 hover:opacity-90 text-white w-8 h-8 rounded font-bold text-lg">−</button>
                            <span className="text-accent font-bold text-base sm:text-lg w-6 text-center">{quantity}</span>
                            <button onClick={() => addItem(menuItem.id)} className="bg-accent-light hover:opacity-90 text-white w-8 h-8 rounded font-bold text-lg">+</button>
                          </div>
                        </div>
                      );
                    })}
          ...
          <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      {adjustments.map(adj => (
                        <div key={adj.id} className="flex justify-between items-center bg-gray-50 rounded p-2 text-sm">
                          <p className="text-gray-600 text-xs">{adj.timestamp.toLocaleString('fr-FR')}</p>
                          <div className="flex items-center gap-2">
                            <span className={`font-semibold ${adj.amount >= 0 ? 'text-accent-light' : 'text-red-500'}`}>{adj.amount >= 0 ? '+' : ''}{adj.amount.toFixed(2)}€</span>
                            <button onClick={() => removeAdjustment(adj.id)} className="text-red-500 hover:opacity-80">✕</button>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="bg-white shadow-md rounded-lg p-3 sm:p-4">
              <div className="flex justify-between items-center cursor-pointer" onClick={() => setShowDivision(!showDivision)}>
                <h4 className="text-accent font-semibold text-sm sm:text-base">DIVISER LE PAIEMENT</h4>
            ...
            <button onClick={onClose} className="w-full bg-accent hover:opacity-90 text-white py-4 rounded-lg font-bold text-lg transition">TERMINER</button>
          </div>
        </div>
      </div>
    </div>
